...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $valor_recebido = isset($_POST['valor_recebido']) ? floatval($_POST['valor_recebido']) : 0;
    $troco = isset($_POST['troco']) ? floatval($_POST['troco']) : 0;
    ...
    try {
        ...
        foreach ($_SESSION['carrinho'] as $produto_id => $quantidade_vendida) {
            ...
            if ($result_produto->num_rows > 0) {
                ...
                // Insere na tabela vendas com informações de desconto e troco
                $sql_venda = "INSERT INTO vendas (produto_id, quantidade_vendida, valor_venda_unitario, valor_original, desconto_aplicado, tipo_desconto, data_venda, forma_pagamento, valor_recebido, troco) 
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt_venda = $conn->prepare($sql_venda);
                $stmt_venda->bind_param("iidddsssdd", $produto_id, $quantidade_vendida, $valor_com_desconto, $valor_original, $desconto_produto, $tipo_desconto_produto, $data_venda, $forma_pagamento, $valor_recebido, $troco);
