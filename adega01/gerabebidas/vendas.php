...
// Carrega taxas do banco de dados (apenas crédito agora)
        ...
        <div class="card p-4 shadow-lg h-100">
            ...
            <form id="form-finalizar-venda" action="finalizar_venda.php" method="POST">
                <div class="mb-3">
                    ...
                    <select class="form-select" id="forma_pagamento" name="forma_pagamento" onchange="toggleTrocoField()">
                    ...
                    <div class="taxa-info mt-1">
                        <small>
                            <!-- REMOVIDA A TAXA DE DÉBITO - APENAS CRÉDITO AGORA -->
                            Taxa Crédito: <?php echo $taxa_credito; ?>%
                        </small>
                    </div>
                </div>

                <!-- CAMPO DE TROCO (APARECE APENAS PARA DINHEIRO) -->
                <div class="mb-3" id="troco_field" style="display: none;">
                    <label for="valor_recebido" class="form-label">💰 Valor Recebido (Dinheiro)</label>
                    <input type="number" step="0.01" class="form-control" id="valor_recebido" name="valor_recebido" placeholder="0,00" oninput="calcularTroco()">
                    <div class="mt-2">
                        <strong>Troco: <span id="troco_calculado" class="text-success">R$ 0,00</span></strong>
...
<script>
// Taxas configuradas (apenas crédito agora)
...
// Função para mostrar/ocultar campo de troco
function toggleTrocoField() {
    ...
    if (formaPagamento === 'Dinheiro') {
        ...
        document.getElementById('troco_calculado').textContent = 'R$ 0,00';
        document.getElementById('troco').value = '0';
    }
    
    calcularTaxa(); // Recalcula taxa também
}

// Função para calcular o troco
function calcularTroco() {
    const valorRecebido = parseFloat(document.getElementById('valor_recebido').value) || 0;
    const total = parseFloat(document.getElementById('total_valor').textContent.replace('R$ ', '').replace('.', '').replace(',', '.'));
    ...
    document.getElementById('troco_calculado').textContent = 'R$ ' + troco.toFixed(2).replace('.', ',');
...
// Chamar toggleTrocoField quando a página carregar
